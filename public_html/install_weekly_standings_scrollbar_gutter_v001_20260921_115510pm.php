...
const EXPECTED_SOURCE_VERSION='v077';
const TARGET_VERSION='v078';

$docRoot=rtrim((string)($_SERVER['DOCUMENT_ROOT']??__DIR__),'/\\');
$target=$docRoot.'/race_results/weekly_standings.php';
$backupDir=$docRoot.'/_installer_backups/weekly_standings_scrollbar_gutter_20260921_115510pm';
...
function build_candidate(string $source,array &$errors):string{
    ...
    $out=replace_once($out,' * VERSION: v077',' * VERSION: v078','Version header',$errors);

    if(preg_match('/ \* LAST MODIFIED: .*? ET\R/',$out,$m)){
        $out=str_replace($m[0],' * LAST MODIFIED: 9/21/2026 11:55:10 pm ET'.PHP_EOL,$out);
    }else{
    ...
    $out=replace_once($out,$cssAnchor,$cssInsert,'Scrollbar gutter CSS anchor',$errors);

    return $out;
}
function candidate_lint(string $candidate,string $dir):array{
    $tmp=@tempnam($dir,'.mrl_ws078_');
    if($tmp===false)$tmp=@tempnam(sys_get_temp_dir(),'mrl_ws078_');
        ...
        if(!is_dir($backupDir)&&!@mkdir($backupDir,0755,true)&&!is_dir($backupDir)){
            ...
            if(empty($l['ok'])){
                ...
                if(empty($l2['ok'])){
                    ...
                    $message='PASS — weekly_standings.php v078 installed and passed PHP lint.';$messageClass='good';
                }
            }
        }
    }
...
if($action==='rollback'&&is_file($backupFile)){
    @copy($backupFile,$target);@chmod($target,0644);
    $message='Rollback complete — weekly_standings.php v077 restored.';$messageClass='good';
}
...
<title>MRL Weekly Standings Scrollbar Gutter</title>
<style>
...
<body><div class="wrap">
<h1>MRL Weekly Standings Scrollbar Gutter</h1>
<div>Installer v001 · generated 9/21/2026 11:55:10 pm ET</div>

<?php if($message!==''):?><div class="card"><?php echo h($message);?></div><?php endif;?>

<div class="card">
<h2>Change</h2>
<p>Keeps <code>scrollbar-gutter: stable</code> exactly as-is, but makes the Chrome/Edge scrollbar track transparent so the reserved space should disappear visually on short pages while still preventing horizontal page movement.</p>
</div>
...
<tr><td>Version</td><td class="<?php echo ($version===EXPECTED_SOURCE_VERSION||$alreadyInstalled)?'pass':'fail';?>"><?php echo ($version===EXPECTED_SOURCE_VERSION||$alreadyInstalled)?'PASS':'FAIL';?></td><td><?php echo h($version);?> → v078</td></tr>
<tr><td>Patch signatures</td><td class="<?php echo ($alreadyInstalled||empty($errors))?'pass':'fail';?>"><?php echo ($alreadyInstalled||empty($errors))?'PASS':'FAIL';?></td><td><?php echo h($alreadyInstalled?'Already installed.':(empty($errors)?'Expected v077 anchors found exactly once.':implode(' | ',$errors)));?></td></tr>
...
<p>
<form method="post" style="display:inline"><input type="hidden" name="action" value="apply"><button class="apply" <?php echo $canApply?'':'disabled';?>>Apply v078</button></form>
