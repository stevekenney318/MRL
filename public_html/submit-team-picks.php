 ...
 * - UI: team.php uses the flag to show a brief "Your picks have been submitted." confirmation.
 * - SAFETY: Failed database writes do not produce a false success confirmation.
 * - PRESERVE: Existing SEG / ADJ / LP / RD submission, deadline and audit behavior.
 *
 ...
 * - FIX: Restored the normal LP/SEG helper functions removed by the damaged temporary patch.
 * - FIX: Restored the complete v007 request/POST setup and opening RD validation branch.
 * - FIX: RD base-row lookup now accepts an active LP row as the Replacement Pick source.
 * - TEST: Preserves only the exact owned Be Like Biff / S1 / AJ / R08 historical deadline bypass.
 * - CLEANUP: Removed temporary fatal-error instrumentation.
 *
...
$currentPickWriteSucceeded = false;
...
if ($exists) {
    ...
    if ($stmtUpdate) {
        ...
        $currentPickWriteSucceeded = mysqli_stmt_execute($stmtUpdate);
    ...
    if ($stmtInsert) {
        ...
        $currentPickWriteSucceeded = mysqli_stmt_execute($stmtInsert);
...
if ($stmtHistory) {
    ...
    $historyWriteSucceeded = mysqli_stmt_execute($stmtHistory);
    mysqli_stmt_close($stmtHistory);
}

if ($currentPickWriteSucceeded && $historyWriteSucceeded) {
    $_SESSION['mrl_pick_submission_success'] = true;
}
