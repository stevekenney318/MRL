...
const TARGET_VERSION = 'v059';

$docRoot = rtrim((string)($_SERVER['DOCUMENT_ROOT'] ?? __DIR__), '/\\');
$target = $docRoot . '/team.php';
$backupDir = $docRoot . '/_installer_backups/team_uid_restore_20260922_052052pm';
...
function build_candidate(string $source, array &$errors): string {
    ...
    if (preg_match('/ \* LAST MODIFIED: .*? ET\R/', $out, $m)) {
        $out = str_replace($m[0], ' * LAST MODIFIED: 9/22/2026 5:20:52 pm ET' . PHP_EOL, $out);
    } else {
...
function candidate_lint(string $candidate, string $dir): array {
    $tmp = @tempnam($dir, '.mrl_team059_');
    if ($tmp === false) {
        $tmp = @tempnam(sys_get_temp_dir(), 'mrl_team059_');
    }
    if ($tmp === false) {
            ...
            if (@file_put_contents($tmpTarget, $candidate, LOCK_EX) === false) {
                ...
                if (empty($tmpLint['ok'])) {
                    ...
                    if (empty($newLint['ok'])) {
                        ...
                        $message = 'PASS — team.php v059 installed and passed PHP lint.';
                        $messageClass = 'good';
                    }
                }
            }
...
<title>MRL Team UID Restore</title>
<style>
...
<body>
<div class="wrap">
<h1>MRL Team UID Restore</h1>
<div class="muted">Installer v001 · generated 9/22/2026 5:20:52 pm ET</div>
...
<p>Restores <code>$uid</code> to an integer immediately after <code>current_user_team_chart.php</code> returns, before the v057 Team Name/pick routing block runs.</p>
<p>This addresses the strict-type failure seen on the guest Team Name form without changing Team Name, LP/RD, pick-window, or database behavior.</p>
</div>
...
<tr><td>Version</td><td class="<?php echo ($version===EXPECTED_SOURCE_VERSION||$alreadyInstalled)?'pass':'fail'; ?>"><?php echo ($version===EXPECTED_SOURCE_VERSION||$alreadyInstalled)?'PASS':'FAIL'; ?></td><td><?php echo h($version); ?> → v059</td></tr>
<tr><td>Patch signatures</td><td class="<?php echo ($alreadyInstalled||empty($errors))?'pass':'fail'; ?>"><?php echo ($alreadyInstalled||empty($errors))?'PASS':'FAIL'; ?></td><td><?php echo h($alreadyInstalled?'Already installed.':(empty($errors)?'Expected v057 post-chart anchor found exactly once.':implode(' | ',$errors))); ?></td></tr>
...
<p>
<form method="post" style="display:inline"><input type="hidden" name="action" value="apply"><button class="apply" <?php echo $canApply?'':'disabled'; ?>>Apply v059</button></form>
