 ...
 *
 * VERSION: v001
 * GENERATED: 8/21/2026 10:02:00 pm America/New_York
 *
 * LIVE MRL ONLY
 *
 * PURPOSE:
...
$backupDir = $root . '/mrl_live_prior_year_connection_backup_20260821_100200pm';
...
if (!$alreadyPatched && empty($errors)) {
    $markers = [
        ["include \"config.php\";", 2, 'Live source has two config.php includes'],
        ["include \"config_mrl.php\";", 1, 'Live source has one config_mrl.php include'],
        ["foreach (\$dbo->query(\$sql) as \$row) {", 4, 'Live source has four direct PDO segment loops'],
        ["'Segment #1'", 1, 'S1 display marker present'],
        ["'Segment #2'", 1, 'S2 display marker present'],
        ["'Segment #3'", 1, 'S3 display marker present'],
        ["'Playoffs'", 1, 'S4 Playoffs display marker present'],
    ];

    foreach ($markers as $m) {
        $count = substr_count($current, $m[0]);
        $ok = ($count === $m[1]);
    ...
    if ($alreadyPatched) {
        ...
        if (empty($errors)) {
            ...
            $postflight = [
                ...
                [
                    'Plain config includes removed',
                    strpos($after, 'include "config.php";') === false
                    && strpos($after, 'include "config_mrl.php";') === false
                ],
                [
                    'Four segment loops use safe query helper',
                    substr_count($after, 'mrl_live_prior_year_query_rows($dbo, $sql, $prevRaceYear,') === 4
                ],
                [
...
<div class="banner">
    <h1>MRL LIVE Prior-Year Connection Fix Installer v001</h1>
    <div class="sub">LIVE MRL ONLY • generated 8/21/2026 10:02:00 pm • DB changes: NONE</div>
</div>

<div class="card">
    <h2>Focused Live Scope</h2>
    <div>
        The Live source was checked separately. This installer changes only the
