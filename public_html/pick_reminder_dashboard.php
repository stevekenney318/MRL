 ...
 *
 * CHANGELOG:
 * v005 (9/6/2026 4:47:42 am)
 * - NEW: Adds Use Current ET Time button for repeated AUTO TEST setup.
 * - CHANGE: Button refreshes Date/Time fields to the actual current America/New_York time.
 * - PRESERVE: Saved test values still persist until the button is deliberately used.
 *
 * v004 (9/6/2026 3:59:58 am)
 * - CHANGE: AUTO TEST uses separate date and time fields instead of datetime-local.
 * - NEW: When no saved AUTO TEST time exists, date/time default to current ET.
 * - CHANGE: Saved AUTO TEST date/time persists after refresh.
 * - NEW: Save Dashboard Settings button added directly in AUTO TEST section.
 * - CHANGE: Existing email-section save button renamed Save Dashboard Settings.
 *
...
function prd_post_cfg(array $c):array{
    ...
    $testDate=trim((string)($_POST['test_auto_date']??''));
    $testTime=trim((string)($_POST['test_auto_time']??''));
...
if($_SERVER['REQUEST_METHOD']==='POST'&&$action==='save_settings'){
    ...
    else{$n=prd_post_cfg($cfg);if((string)($n['test_auto_at']??'')==='__INVALID__'){$msg='AUTO TEST date/time is invalid. Use MM/DD/YYYY and HH:MM AM/PM.';$msgClass='bad';}
    elseif($n['mode']==='AUTO'&&$n['scope']==='LIVE'&&trim((string)($_POST['live_auto_confirm']??''))!=='ENABLE LIVE AUTO'){$msg='LIVE AUTO was not enabled. Type ENABLE LIVE AUTO exactly.';$msgClass='bad';}
...
<?php
$testSavedAt=trim((string)($cfg['test_auto_at']??''));
$testDateValue=date('m/d/Y');
$testTimeValue=date('h:i A');

if($testSavedAt!==''){
    try{
        $testSavedDt=new DateTime($testSavedAt,new DateTimeZone('America/New_York'));
        $testDateValue=$testSavedDt->format('m/d/Y');
        $testTimeValue=$testSavedDt->format('h:i A');
    }catch(Throwable $e){
        // Fall back to current ET date/time for display only.
    }
}
?>
    ...
    <div class="stat">
        ...
        <div class="test-fields">
            <div>
                ...
                <input type="text" id="test_auto_date" name="test_auto_date" value="<?php echo mrlpr_h($testDateValue); ?>" placeholder="MM/DD/YYYY">
            </div>
            <div>
                <label class="small">Time</label>
                <input type="text" id="test_auto_time" name="test_auto_time" value="<?php echo mrlpr_h($testTimeValue); ?>" placeholder="HH:MM AM">
            </div>
        </div>
        <div style="margin-top:10px">
            <button class="btn blue" type="button" id="use_current_et_time">Use Current ET Time</button>
        </div>
        <p class="small">When no test has been saved, these start at the current ET date/time. For another test, click Use Current ET Time, then usually just change the minutes.</p>
    </div>
</div>
<?php
...
<div class="save-row">
    <button class="btn blue" type="submit" name="action" value="save_settings">Save Dashboard Settings</button>
</div>
</div>
...
<?php echo mrlpr_h($previewBody);?></div></div></div><div style="margin-top:14px"><button class="btn blue" type="submit" name="action" value="save_settings">Save Dashboard Settings</button></div></div>
...
<div class="small" style="text-align:right">pick_reminder_dashboard.php | VERSION v005</div></div>
<script>
(function(){
    'use strict';

    const btn = document.getElementById('use_current_et_time');
    const dateField = document.getElementById('test_auto_date');
    const timeField = document.getElementById('test_auto_time');

    if (!btn || !dateField || !timeField) return;
    ...
    btn.addEventListener('click', function(){
        ...
        timeField.value =
            ...
            String(values.dayPeriod || '').toUpperCase();
    });
})();
</script>
