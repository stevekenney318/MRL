...
const MRL_INSTALLER_VERSION = 'v002';
...
function preflight(string $target): array {
    ...
    $checks[] = [
        ...
        exact_count($content, '    VERSION: v002') === 1,
        ''
    ];

    $checks[] = [
        'Expected old filename stamp code found once',
        exact_count($content, old_stamp_line()) === 1,
        ''
    ];

    $checks[] = [
        'Expected old filename validator found once',
        exact_count($content, old_validator_line()) === 1,
        ''
    ];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'apply') {
    ...
    if (!$pre['ok']) {
        ...
        if (!$patch['ok']) {
            ...
            $updated = str_replace('9/7/2026 2:05:27 pm', '__DISPLAY_REAL__', $patch['content']);
            ...
            if (!write_text($backup, $original)) {
                ...
                if (!write_text($temp, $updated)) {
                    ...
                    if (!$lint['ok']) {
                        ...
                        $checks = [
                            ['Header VERSION v003', exact_count($installed, '    VERSION: v003') === 1, ''],
                            ['24-hour filename stamp installed', exact_count($installed, new_stamp_line()) === 1, ''],
                            ['Old filename stamp removed', exact_count($installed, old_stamp_line()) === 0, ''],
                            ['Old + new filename validation installed', exact_count($installed, new_validator_line()) === 1, ''],
                        ];
                        ...
                        if ($all) {
                            ...
                            $message = 'Postflight failed. Original v002 restored automatically.';
                            $messageClass = 'bad';
                        }
                    }
                }
...
.good{background:#103b27;border:1px solid #2f9b63}.bad{background:#4b1d1d;border:1px solid #c04b4b}.info{background:#173246;border:1px solid #387ba8}
...
<div class="small" style="margin-top:5px">Old am/pm ZIP names remain supported for Download/Delete. No other backup behavior is changed.</div>
</div>
