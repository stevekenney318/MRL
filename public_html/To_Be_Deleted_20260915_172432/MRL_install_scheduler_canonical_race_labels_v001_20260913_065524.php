...
/**
 * MRL_install_scheduler_canonical_race_labels_v001.php
 *
 * Generated: 9/13/2026 6:55:24 am ET
 *
 * PURPOSE
 * -------
 * Follow-up to the canonical short-race-label installation.
 *
 * The canonical schedule now correctly carries mrl_race_name (for example,
 * "World Wide Tech"), but cron_master_scheduler.php v014 still builds its
 * display label from short_name before race_name. That leaves scheduler-driven
 * displays such as:
 *   - Race Scheduler dashboard panel
 *   - MRL At a Glance
 * showing the old ESPN-shortened value "World".
 *
 * THIS INSTALLER
 * --------------
 * - Updates cron_master_scheduler.php v014 -> v015.
 * - Makes scheduler display state prefer mrl_race_name for compact race labels.
 * - Persists canonical mrl_race_name into scheduler next_race state.
 * - Immediately repairs the current _scheduler/state.json next-race display
 *   from _race_results_schedule.json, so the display does not need to wait for
 *   the next scheduler heartbeat.
 * - Makes NO database writes.
 *
 * Production-safe behavior:
 * - Exact preflight checks before any source modification.
 * - Timestamped backups of every file changed.
 * - Atomic file writes.
...
function race_number_from_row(array $race): int
{
    ...
    if ($n <= 0) {
        $n = (int)($race['race_number'] ?? 0);
    }
    if ($n <= 0) {
        $n = (int)($race['schedule_sequence'] ?? 0);
    }
    return $n;
}
...
function find_schedule_race(array $schedule, int $raceNumber): ?array
{
    foreach (['mrl_points_races', 'races'] as $key) {
        if (empty($schedule[$key]) || !is_array($schedule[$key])) {
            continue;
        }
        ...
        foreach ($schedule[$key] as $race) {
            ...
            if (race_number_from_row($race) === $raceNumber) {
                return $race;
            }
        }
    }
...
function repair_scheduler_state(array $state, array $schedule, array &$detail): array
{
    ...
    $detail[] = 'Runtime scheduler state: '
        . ($oldLabel !== '' ? $oldLabel : ($code . ' ' . $oldShort))
        . ' -> ' . $code . ' ' . $canonical;
...
if ($cronReadable) {
    ...
    $preflight[] = [
        ...
        empty($errors) ? 'v014 expected signatures found' : implode('; ', $errors)
    ];
}
...
if ($scheduleReadable) {
    ...
    if (!is_array($schedule)) {
        $errors[] = 'Canonical schedule JSON is invalid.';
    }
}
    ...
    if (!$canInstall) {
        ...
        try {
            ...
            $newCron = replace_exact(
                ...
                '* LAST MODIFIED: 9/13/2026 6:55:24 am',
                1,
                'Last modified'
            );
            ...
            $newCron = replace_exact(
                ...
                " * CHANGELOG:\n * v013 (6/21/2026)",
                " * CHANGELOG:\n"
                . " * v015 (9/13/2026 6:55:24 am)\n"
                . " * - FIX: Race scheduler display labels now prefer canonical mrl_race_name before ESPN short_name/race_name.\n"
                . " * - FIX: Scheduler next_race state now stores canonical compact short_name and mrl_race_name for downstream displays.\n"
            ...
            $newCron = replace_exact(
                $newCron,
                "const CMS_VERSION = 'v014';",
                "const CMS_VERSION = 'v015';",
                1,
                'CMS_VERSION'
            );
            ...
            $newCron = replace_exact(
                ...
                1,
                'next_race state canonical name'
            );
            ...
            if (
                ...
                || strpos($newCron, "\$nextRace['mrl_race_name'] ?? \$nextRace['short_name'] ?? \$nextRace['race_name']") === false
            ) {
            ...
            if (is_array($state)) {
                ...
                foreach ($runtimeMessages as $message) {
                    $installMessages[] = $message;
                }
            } else {
                $installMessages[] = 'Scheduler state was not present; next normal scheduler run will create canonical display state.';
            }
            ...
            if (is_file($stateFile)) {
                ...
                if (is_array($verifyState)) {
                    ...
                    if (!empty($verifyNext)) {
                        ...
                        $verifyRace = $verifyRaceNumber > 0
                            ? find_schedule_race($schedule, $verifyRaceNumber)
                            : null;
...
<title>MRL Scheduler Canonical Race Labels v001</title>
<style>
    :root {
        color-scheme: dark;
        --bg: #121212;
...
<div class="panel">
    <h1>MRL Scheduler Canonical Race Labels v001</h1>
    <p>Generated <?= h($generated) ?> · Production-safe scheduler/state repair · No DB writes</p>
</div>

<div class="panel">
    <h2>What this installs</h2>
    <ul>
        <li>Updates <code>cron_master_scheduler.php</code> v014 → v015.</li>
        <li>Makes scheduler display labels prefer canonical <code>mrl_race_name</code>.</li>
        <li>Repairs current scheduler state immediately from the canonical schedule.</li>
        <li>Fixes the shared upstream source used by Race Scheduler and MRL At a Glance.</li>
        <li>Leaves scheduling cadence, race matching, monitor/revision logic, and databases untouched.</li>
    </ul>
</div>
...
<div class="panel">
    <p class="pass">
        Done. Reload Race Scheduler and MRL At a Glance. Both should now show the canonical short race name.
    </p>
</div>
...
<div class="panel">
    <form method="post">
        <button type="submit" name="install" value="1" <?= $canInstall ? '' : 'disabled' ?>>
            Install Fix
        </button>
    </form>
</div>
