...
/**
 * install_team_header_refinement_v002.php
 *
 * VERSION: v002
 * LAST MODIFIED: 8/28/2026 3:45:40 pm
 *
 * PURPOSE:
 * Same Team masthead refinement as v001, but with a more robust icon-line patch.
 *
 * WHY v002:
 * v001 preflight correctly stopped because the icon line did not match the expected
 * literal emoji sequence byte-for-byte. No site file was changed.
 *
 * TARGET:
 *   /team.php v036 -> v037
 *
 ...
 * - every ordinary patch anchor must exist exactly once
 * - icon line is matched structurally by its PHP/name/dropdown signature
...
function replace_user_icon_line(string $source, array &$log): ?string {
    $pattern = '/^[ \t]*[^\r\n]*<\?php echo teampage_h\(\$first_name\); \?>\s*▾[ \t]*$/mu';

    $count = preg_match_all($pattern, $source, $matches);
    if ($count !== 1) {
        $log[] = 'PATCH FAIL: checkered flag icon structural match expected 1 line, found ' . (int)$count . '.';
        return null;
    }

    $matchedLine = (string)$matches[0][0];
    $indent = '';
    if (preg_match('/^([ \t]*)/u', $matchedLine, $m) === 1) {
        $indent = (string)$m[1];
    }

    $newLine = $indent . '🏁 <?php echo teampage_h($first_name); ?> ▾';
    $replaced = preg_replace($pattern, $newLine, $source, 1, $replaceCount);

    if (!is_string($replaced) || $replaceCount !== 1) {
        $log[] = 'PATCH FAIL: checkered flag icon replacement did not complete exactly once.';
        return null;
    }

    $log[] = 'PATCH PASS: checkered flag icon (structural line match).';
    return $replaced;
}
...
function build_v037(string $source, array &$log): ?string {
    ...
    $s = replace_user_icon_line($s, $log);
...
<head>
<meta charset="utf-8">
<title>Install Team Header Refinement v002</title>
<style>
...
<div class="card">
<h1>Team Header Refinement v002</h1>
<p>Same refinement, with a robust structural match for the user icon line.</p>
</div>
...
<li>Current user icon becomes 🏁.</li>
...
<li>All masthead text uses normal weight instead of bold.</li>
