 ...
 *
 * VERSION: v003
 * LAST MODIFIED: 8/30/2026 8:31:39 am
 *
 * PURPOSE:
...
if ($applyAttempted) {
    if (!$preflightReady) {
        ...
        try {
            ...
            if (!mysqli_query(
                $dbconnect,
                "UPDATE `users` SET `userEmail`='{$tempEsc}' WHERE `userID`=0 LIMIT 1"
            )) {
...
<p><strong>Installer:</strong> v003 &nbsp; | &nbsp; <strong>Generated:</strong> 8/30/2026 8:31:39 am America/New_York</p>
...
<p class="muted">During Apply, userID 0 temporarily becomes mrl@google.com only long enough to free the UNIQUE Gmail key. userID 999 receives manliusracingleague@gmail.com, then userID 0 is deleted before commit.</p>
</div>
