 ...
 *
 * VERSION: v002
 * LAST MODIFIED: 8/30/2026 8:24:12 am
 *
 * PURPOSE:
 ...
 *   4. Copy users.userID 0 -> explicit userID 999, preserving every other field.
 *   5. Verify copied user payload is identical except for userID.
 *   6. Update user_picks, user_picks_history, and user_teams from 0 -> 999.
 *   7. Verify exact physical row counts before source-user deletion.
 *   8. Delete users.userID 0.
 *   9. Verify physical tables AND the three views.
 *  10. COMMIT only if every check passes; otherwise ROLLBACK.
 *
...
if ($applyAttempted) {
    if (!$preflightReady) {
        ...
        try {
            ...
            /*
             * Copy users row with only the primary key changed.
             */
            ...
            if (!$userCopyHashMatch) {
                throw new RuntimeException('Copied users row differs in one or more non-ID fields.');
            }
...
<p><strong>Installer:</strong> v002 &nbsp; | &nbsp; <strong>Generated:</strong> 8/30/2026 8:24:12 am America/New_York</p>
...
<form method="post" onsubmit="return confirm('Migrate the MRL test account from userID 0 to 999 now? This will change 44 physical database rows inside one transaction.');">
