...
function mrl_audit_starts_with(string $haystack, string $needle): bool
{
...
function mrl_audit_is_within(string $path, string $parent): bool
{
    ...
    return $path === $parent || mrl_audit_starts_with($path, $parent . '/');
}
...
function mrl_audit_relative(string $path, string $root): string
{
    ...
    if (mrl_audit_starts_with($path, $root . '/')) {
...
function mrl_audit_severity(string $file, string $docRoot, string $accountRoot): string
{
    ...
    if (mrl_audit_starts_with($norm, rtrim(str_replace('\\', '/', $accountRoot), '/') . '/_mrl_backups/')) {
        return 'MEDIUM';
    }

    if (mrl_audit_starts_with($norm, rtrim(str_replace('\\', '/', $accountRoot), '/') . '/_mrl_private/')) {
        return 'INFO';
    }
...
<title>MRL Credential Exposure Audit v002</title>
<style>
...
<div class="wrap">
    ...
    <div>Version v002 · Read-only · <?php echo mrl_audit_h(date('n/j/Y g:i:s a')); ?> America/New_York</div>
