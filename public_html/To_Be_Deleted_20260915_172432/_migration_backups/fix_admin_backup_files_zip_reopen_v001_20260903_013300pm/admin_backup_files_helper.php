...
/*
    ...
    - Verifies that this server preserves source file mtimes in ZIP entries before allowing a backup.
    - Includes _backup_manifest.json with path, type, exact Unix mtime, size, permissions, SHA-256.
...
function mrlfb_zip_timestamp_test(string $workDir): array {
    if (!class_exists('ZipArchive')) {
        return [false, 'ZipArchive is not available.'];
    }
    if (!mrlfb_ensure_dir($workDir)) {
        return [false, 'Timestamp test folder is not writable.'];
    }

    $token = 'mtime_test_' . bin2hex(random_bytes(4));
    $testFile = $workDir . DIRECTORY_SEPARATOR . $token . '.txt';
    $testZip  = $workDir . DIRECTORY_SEPARATOR . $token . '.zip';
    $entryName = $token . '.txt';

    $wanted = time() - 7200;
    if (($wanted % 2) !== 0) $wanted--; // ZIP's traditional timestamp has 2-second resolution.

    $ok = @file_put_contents($testFile, "MRL timestamp test\n") !== false;
    if ($ok) $ok = @touch($testFile, $wanted);

    $stored = 0;
    if ($ok) {
        $zip = new ZipArchive();
        $res = $zip->open($testZip, ZipArchive::CREATE | ZipArchive::OVERWRITE);
        if ($res === true) {
            $ok = $zip->addFile($testFile, $entryName);
            if ($ok && method_exists($zip, 'setMtimeName')) {
                @ $zip->setMtimeName($entryName, $wanted);
            }
            $zip->close();
        } else {
            $ok = false;
    ...
    if ($ok) {
        $zip = new ZipArchive();
        if ($zip->open($testZip) === true) {
            $stat = $zip->statName($entryName);
            if (is_array($stat)) $stored = (int)($stat['mtime'] ?? 0);
            $zip->close();
        }
    }
    ...
    $pass = $ok && $stored > 0 && abs($stored - $wanted) <= 2;
    $detail = $pass
        ? 'PASS — actual ZIP entry retained the source file modification time.'
        : 'FAIL — ZIP entry did not retain the source file modification time.';
...
if ($ajax !== '') {
    ...
    if ($ajax === 'preflight') {
        ...
        [$mtimeOk,$mtimeDetail]=mrlfb_zip_timestamp_test($stateDir);
        if (!$mtimeOk) mrlfb_json(['ok'=>false,'error'=>$mtimeDetail],500);
        mrlfb_json([
            'ok'=>true,
            'source'=>$sourceDir,
            ...
            'dirs'=>$scan['dirs'],
            'bytes'=>$scan['bytes'],
            'bytes_label'=>mrlfb_fmt_bytes($scan['bytes']),
            'free_bytes'=>(int)$free,
            'free_label'=>mrlfb_fmt_bytes((int)$free),
            'mtime_verified'=>true,
            'mtime_detail'=>$mtimeDetail,
            'mtime_api'=>method_exists('ZipArchive','setMtimeName') ? 'setMtimeName available' : 'legacy addFile timestamp preservation verified',
            'excluded'=>$excludedTop
        ]);
    }
...
<div class="card"><div class="title">Backup Scope</div><p class="muted">Source: <span class="mono"><?php echo mrlfb_h($sourceDir); ?></span><br>Destination: <span class="mono"><?php echo mrlfb_h($backupRoot); ?></span><br>Excluded completely: <span class="mono">wp-admin / wp-includes / wp-content</span></p><p class="muted">Before a backup is allowed, the page performs a real ZIP round-trip test to verify that this server retains original <strong>file modification times</strong>. A <span class="mono">_backup_manifest.json</span> is also stored inside every ZIP with exact Unix mtime, size, permissions and SHA-256 for every file and folder.</p></div>
