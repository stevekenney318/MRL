 ...
 *
 * VERSION: v003
 * CREATED: 9/9/2026 2:40:33 am ET
 *
 * PURPOSE:
...
const MRL_INSTALLER_VERSION = 'v003';
...
$manifestPath = $backupDir . '/team_chart_lp_consistency_v003_manifest.json';
...
PHP;
    ...
    $needleEffective = "\$effectiveRaceLabel = tc_effective_race_label(\$row['effective_race'] ?? 0);";
    $countEffective = exact_count($c, $needleEffective);
    if ($countEffective !== 2) {
        return ['ok'=>false, 'content'=>$c, 'error'=>'team_chart effective-note signature count = ' . $countEffective . ' (expected 2)'];
    }
    $c = str_replace(
        $needleEffective,
...
PHP;
    $r = replace_once($c, $needle, $replacement, 'team_chart LP time HTML contextual');
    if (!$r['ok']) return $r;
    $c = $r['content'];
...
PHP;
    ...
    $needleEffective = "\$effectiveRaceLabel = csc_effective_race_label(\$row['effective_race'] ?? 0);";
    $countEffective = exact_count($c, $needleEffective);
    if ($countEffective !== 2) {
        return ['ok'=>false, 'content'=>$c, 'error'=>'current_segment effective-note signature count = ' . $countEffective . ' (expected 2)'];
    }
    $c = str_replace(
        $needleEffective,
...
PHP;
    $replacement = <<<'PHP'
            ...
            echo "</tr>";
PHP;
    $r = replace_once($c, $needle, $replacement, 'current_segment LP timestamp output contextual');
    if (!$r['ok']) return $r;
    $c = $r['content'];
