 ...
 * - CHANGE: Reminder mail now uses the existing USER::send_mail() Gmail/PHPMailer path.
 * - CHANGE: MRL Gmail is the visible To: address and team email(s) are BCC recipients.
 * - NEW: Default reminder message includes the team name.
 * - NEW: One-time AUTO TEST timestamp support for ID 999.
 *
...
function mrlpr_default_config(): array {
    return [
        ...
        'body_template'=>"Just a reminder that {{team_name}} has not submitted its picks for {{year}} {{segment_name}}.\nThey are due soon — {{deadline}}.\n{{team_page}}\nThanks, Steve",
        ...
        'to_email'=>'manliusracingleague@gmail.com',
        'auto_window_minutes'=>10,
        'test_auto_enabled'=>false,
        'test_auto_at'=>'',
        'updated_at'=>null
    ];
}
function mrlpr_load_config(): array {
    ...
    $c['test_auto_enabled']=!empty($c['test_auto_enabled']);
    $c['test_auto_at']=trim((string)($c['test_auto_at']??''));
...
function mrlpr_send_user(mysqli $db,array $r,array $cfg,array $ctx,string $kind,?int $off=null,array $extra=[]): array {
    $uid=(int)($r['userID']??0);
    $team=trim((string)($r['teamName']??$r['userName']??('User '.$uid)));
    $emails=(array)($r['emails']??[]);
    ...
    if(!mrlpr_recheck_missing($db,$uid,(string)$ctx['year'],(string)$ctx['segment'])){
        $base['result']='SKIPPED_ALREADY_SUBMITTED';
        mrlpr_append_log($base);
        return $base;
    }

    if(!$emails){
        $base['result']='FAILED_NO_EMAIL';
        mrlpr_append_log($base);
        return $base;
    }

    if(!class_exists('USER')){
        $classFile=__DIR__.'/class.user.php';
        if(is_file($classFile)) require_once $classFile;
    }
    if(!class_exists('USER')){
        $base['result']='FAILED_MAILER_UNAVAILABLE';
        mrlpr_append_log($base);
        return $base;
    }
    ...
    $tc=[
        ...
        'deadline'=>$ctx['deadline_display'],
        'team_name'=>$team,
        'team_page_url'=>$cfg['team_page_url']
    ];

    $subject=mrlpr_render_template((string)$cfg['subject_template'],$tc,false);
    $raw=mrlpr_render_template((string)$cfg['body_template'],$tc,true);
    $link='<a href="'.mrlpr_h((string)$cfg['team_page_url']).'">Team Page</a>';
    $raw=str_replace($link,'__LINK__',$raw);
    $html=nl2br(mrlpr_h($raw));
    $html=str_replace('__LINK__',$link,$html);

    $toEmail=(string)($cfg['to_email']??'manliusracingleague@gmail.com');

    try{
        $mailer=new USER();
        $sent=(bool)$mailer->send_mail($toEmail,$html,$subject,$emails);
    }catch(Throwable $e){
        $sent=false;
    ...
    $base['to']=$toEmail;
    $base['bcc']=$emails;
    $base['result']=$sent?'SENT':'FAILED';
    $base['subject']=$subject;
    $base['body_text']=mrlpr_render_template((string)$cfg['body_template'],$tc,false);
    mrlpr_append_log($base);
    return $base;
}

function mrlpr_test_auto_sent(string $scheduledAt,int $uid=MRL_PR_TEST_UID): bool {
    foreach(mrlpr_read_log(5000) as $r){
        if(($r['send_kind']??'')!=='AUTO_TEST') continue;
        if((int)($r['userID']??-1)!==$uid) continue;
        if((string)($r['test_auto_at']??'')!==$scheduledAt) continue;
