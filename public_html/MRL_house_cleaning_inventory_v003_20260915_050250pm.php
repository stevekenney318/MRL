 ...
 *
 * VERSION: v003
 * LAST MODIFIED: 9/15/2026 5:02:50 pm
 *
 * PHASE 1 / READ-ONLY.
 *
 * v003:
 * - User-approved entire _mrl_installer_backups/ tree as CONFIRMED CLEANUP.
 * - User-approved entire _migration_backups/ tree as CONFIRMED CLEANUP.
 * - Keeps exact known-artifact manifest from v002.
 * - Retains LIKELY CLEANUP / REVIEW / PROTECTED separation.
 * - Still contains NO delete/move/rename/edit/DB-write behavior.
 *
 * v002:
 * - Added exact KNOWN ARTIFACT manifest for positively identified generated
 *   installers and one-time utilities.
 * - Added PROTECTED handling for active backup helpers and db_backups.
 */

date_default_timezone_set('America/New_York');
const MRL_HOUSE_CLEAN_VERSION = 'v003';
...
function classify_file(string $relative,int $size,int $mtime,array $knownCleanup,array $protectedExact): ?array {
    ...
    if (strpos($relLower,'_migration_backups/')===0) {
        return make_item(
            $relative,$size,$mtime,
            'Approved migration backup tree',
            'CONFIRMED CLEANUP',
            'User-approved entire _migration_backups/ tree for cleanup.'
        );
    }
    ...
    if (strpos($relLower,'_mrl_installer_backups/')===0) {
        return make_item(
            $relative,$size,$mtime,
            'Approved installer backup tree',
            'CONFIRMED CLEANUP',
            'User-approved entire _mrl_installer_backups/ tree for cleanup.'
        );
    }
...
if ($run) {
    ...
    if (($_GET['export'] ?? '')==='json') {
        ...
        header('Content-Disposition: attachment; filename="MRL_house_cleaning_inventory_v003_'.date('Ymd_His').'.json"');
...
if ($scan && $summary) {
    $lines=[
        'MRL HOUSE CLEANING INVENTORY v003',
...
<title>MRL House Cleaning Inventory v003</title>
<style>
...
<body><div class="wrap">
<h1>MRL House Cleaning Inventory v003</h1>

<div class="readonly">READ ONLY — still no delete button. v003 adds the two user-approved backup trees to CONFIRMED CLEANUP.</div>

<div class="panel">
<h2>What changed</h2>
<div class="note">
<strong>Confirmed cleanup now includes:</strong> exact known artifacts plus the complete <code>_mrl_installer_backups/</code> and <code>_migration_backups/</code> trees.
Those two folder trees are now treated as user-approved cleanup groups. Everything else remains likely, review, or protected unless explicitly confirmed.
</div>
<ul>
<li><strong>CONFIRMED CLEANUP</strong> = exact known artifact or item inside one of the two user-approved backup trees.</li>
...
<div class="panel small">FILE: <?php echo h(basename(__FILE__)); ?> | VERSION: v003 | LAST MODIFIED: 9/15/2026 5:02:50 pm</div>
