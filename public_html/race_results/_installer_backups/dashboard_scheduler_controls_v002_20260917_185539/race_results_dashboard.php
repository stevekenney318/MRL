 ...
 *
 * VERSION: v023
 * LAST MODIFIED: 9/17/2026 6:22:10 pm
 *
 * CHANGELOG:
 *
 ...
 *
 * v020 (6/21/2026)
 *   - FIX: Dashboard next-run pills now use auto_race_monitor/auto_revision_monitor schedule state instead of falling back to a generic 60-minute interval.
 *   - FIX: Monitor tab can display the scheduler-selected next race after a final handoff, even when the stored monitor state still reflects the just-finished race.
 *
 * v019 (6/21/2026)
 *   - NEW: Adds NASCAR At a Glance info-only panel inside Monitor tab Race Status.
 *   - NOTE: NASCAR cache is display-only and does not drive scoring/final/revision logic.
 *
 * v018 (6/14/2026)
...
const RACE_RESULTS_DASHBOARD_VERSION = 'v023';
...
foreach ($tasks as $taskNameForStatus => $taskForStatus) {
    ...
    if ($typeForStatus === 'daily_times') {
        $taskStatusRows[$taskNameForStatus] = sd_daily_task_status($taskForStatus, $taskStateForStatus, $now, $tz);
    } elseif ($typeForStatus === 'auto_race_monitor') {
        $taskStatusRows[$taskNameForStatus] = sd_auto_race_task_status($taskForStatus, $taskStateForStatus, $now, $tz);
    } elseif ($typeForStatus === 'auto_revision_monitor') {
        $taskStatusRows[$taskNameForStatus] = sd_auto_revision_task_status($taskForStatus, $taskStateForStatus, $now, $tz);
    } else {
        $taskStatusRows[$taskNameForStatus] = sd_interval_task_status($taskForStatus, $taskStateForStatus, $now, $tz);
    }
}
...
function rr_dash_monitor_status_from_state(array $yearState): array
{
    ...
    return [
        ...
        'is_final' => !empty($statusRow['is_final']) || !empty($statusRow['final_email_sent']),
        'final_email_sent' => !empty($statusRow['final_email_sent']),
        'latest_final_race_url' => (string)($statusRow['latest_final_race_url'] ?? ''),
        'latest_final_race_name' => (string)($statusRow['latest_final_race_name'] ?? ''),
...
if (!empty($monitorCurrentRaceStatus['is_final'])) {
    $raceTaskStateForDisplay = isset($stateTasks['race_results_monitor']) && is_array($stateTasks['race_results_monitor']) ? $stateTasks['race_results_monitor'] : [];
    $raceAutoForDisplay = isset($raceTaskStateForDisplay['auto_schedule']) && is_array($raceTaskStateForDisplay['auto_schedule']) ? $raceTaskStateForDisplay['auto_schedule'] : [];
    $nextRaceForDisplay = isset($raceAutoForDisplay['next_race']) && is_array($raceAutoForDisplay['next_race']) ? $raceAutoForDisplay['next_race'] : [];
    $nextRaceLabelForDisplay = trim((string)($nextRaceForDisplay['mrl_race_code'] ?? '') . ' ' . (string)($nextRaceForDisplay['short_name'] ?? $nextRaceForDisplay['race_name'] ?? ''));
    $nextRaceStartForDisplay = (string)($nextRaceForDisplay['start_at'] ?? '');
    $nextRaceStartTsForDisplay = 0;
    if ($nextRaceStartForDisplay !== '') {
        $nextRaceStartParsedForDisplay = strtotime($nextRaceStartForDisplay);
        $nextRaceStartTsForDisplay = $nextRaceStartParsedForDisplay === false ? 0 : (int)$nextRaceStartParsedForDisplay;
    }

    if ($nextRaceLabelForDisplay !== '' && $nextRaceStartTsForDisplay > $now->getTimestamp()) {
        $monitorCurrentRaceStatus['label'] = 'Next Race';
        $monitorCurrentRaceStatus['race_name'] = $nextRaceLabelForDisplay;
        $monitorCurrentRaceStatus['status'] = 'Scheduled - ' . (string)($nextRaceForDisplay['start_text'] ?? $nextRaceStartForDisplay);
        $monitorCurrentRaceStatus['checked_at'] = rr_dash_display_datetime((string)($raceAutoForDisplay['generated_at'] ?? ''));
        $monitorCurrentRaceStatus['url'] = '';
        $monitorCurrentRaceStatus['can_open_race_page'] = false;
        $monitorCurrentRaceStatus['owned_by'] = 'cron_master_scheduler';
        $monitorCurrentRaceStatus['found'] = true;
    }
}
        ...
        <div class="scheduler-nav-control">
            ...
            <form method="post" class="scheduler-toggle-form">
                ...
                <button class="scheduler-switch <?php echo $raceSchedulerEnabled ? 'is-on' : 'is-off'; ?>"
                        ...
                        title="Race monitor: <?php echo $raceSchedulerEnabled ? 'ON — click to turn OFF' : 'OFF — click to turn ON'; ?>"></button>
            </form>
        </div>
        ...
        <div class="scheduler-nav-control">
            ...
            <form method="post" class="scheduler-toggle-form">
                ...
                <button class="scheduler-switch <?php echo $revisionSchedulerEnabled ? 'is-on' : 'is-off'; ?>"
                        ...
                        title="Revision monitor: <?php echo $revisionSchedulerEnabled ? 'ON — click to turn OFF' : 'OFF — click to turn ON'; ?>"></button>
            </form>
        </div>
    </div>
</div>
