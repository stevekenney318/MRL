 ...
 * CHANGELOG:
 * v013 (6/21/2026)
 * - NEW: Updates optional NASCAR At a Glance cache after race_results_monitor runs.
 * - NOTE: NASCAR data remains info-only and does not drive MRL scoring/final/revision logic.
 *
 * v012 (6/14/2026)
...
foreach ($tasks as $taskName => $task) {
    ...
    if ($taskName === 'race_results_monitor' && (int)$result['exit_code'] === 0) {
        cms_mrl_nascar_cache_after_monitor_run($baseDir, $year, $state, $taskName, $logFile);
    }
...
function cms_mrl_nascar_cache_after_monitor_run(string $baseDir, int $year, array &$state, string $taskName, string $logFile): void
{
    $helper = rtrim($baseDir, '/\\') . '/mrl_nascar_live_helper.php';
    if (!is_file($helper)) {
        $state['tasks'][$taskName]['nascar_live_cache'] = [
            'ok' => false,
            'updated_at' => cms_now_string(),
            'message' => 'mrl_nascar_live_helper.php not found.',
        ];
        cms_log($logFile, 'NASCAR CACHE skipped helper missing');
        return;
    }

    try {
        require_once $helper;
        if (!function_exists('mrl_nascar_live_update_cache')) {
            throw new RuntimeException('mrl_nascar_live_update_cache() not available.');
        }
        ...
        $state['tasks'][$taskName]['nascar_live_cache'] = [
            'ok' => !empty($status['ok']),
            'updated_at' => cms_now_string(),
            'race_id' => (int)($status['race_id'] ?? 0),
            'race_name' => (string)($status['race_name'] ?? ''),
            'flag_label' => (string)($status['flag_label'] ?? ''),
            'stage_label' => (string)($status['stage_label'] ?? ''),
            'lap_label' => (string)($status['lap_label'] ?? ''),
            'elapsed_label' => (string)($status['elapsed_label'] ?? ''),
            'message' => (string)($status['message'] ?? ''),
        ];
        cms_log($logFile, 'NASCAR CACHE updated ok=' . (!empty($status['ok']) ? 'YES' : 'NO') . ' race_id=' . (string)($status['race_id'] ?? '') . ' lap=' . (string)($status['lap_label'] ?? ''));
    } catch (Throwable $e) {
        $state['tasks'][$taskName]['nascar_live_cache'] = [
            'ok' => false,
            'updated_at' => cms_now_string(),
        ...
        cms_log($logFile, 'NASCAR CACHE error=' . cms_compact_log_text($e->getMessage()));
    }
}
...
function cms_monitor_final_complete_info(array $monitorState, int $year): array
{
    ...
    $finalCheck = isset($yearState['final_check']) && is_array($yearState['final_check'])
        ...
        : [];
    $handoff = isset($yearState['final_handoff']) && is_array($yearState['final_handoff'])
        ? $yearState['final_handoff']
        : [];

    $isFinal = !empty($finalCheck['is_final'])
        || !empty($raceStatus['is_final'])
        || !empty($currentRaceStatus['is_final'])
        || !empty($yearState['final_sent_for_url'])
        || !empty($handoff['email_sent'])
        || !empty($handoff['snapshot_file']);
    ...
    if ($raceId === '') $raceId = (string)($handoff['race_id'] ?? '');
    if ($raceId === '') $raceId = (string)($currentRaceStatus['race_id'] ?? '');

    $raceName = (string)($handoff['race_name'] ?? '');
    if ($raceName === '') $raceName = (string)($currentRaceStatus['full_race_name'] ?? '');
    if ($raceName === '') $raceName = (string)($currentRaceStatus['race_name'] ?? '');
    if ($raceName === '') $raceName = (string)($raceStatus['full_race_name'] ?? '');
    if ($raceName === '') $raceName = (string)($raceStatus['race_name'] ?? '');
    if ($raceName === '') $raceName = (string)($raceStatus['latest_final_race_name'] ?? '');

    $raceFolder = (string)($handoff['race_folder'] ?? '');
    $raceNumber = (string)($handoff['race_number'] ?? '');

    $checkedAt = (string)($handoff['anchor_at'] ?? '');
    if ($checkedAt === '') $checkedAt = (string)($handoff['email_sent_at'] ?? '');
    if ($checkedAt === '') $checkedAt = (string)($finalCheck['checked_at'] ?? '');
...
{
    $nextRace = isset($raceSchedule['next_race']) && is_array($raceSchedule['next_race'])
        ? $raceSchedule['next_race']
        : [];
    ...
    if (!empty($nextRace) && cms_race_entry_matches_final($nextRace, $final)) {
        ...
        if (!empty($futureRace)) {
            ...
            $futureRace['_scheduler_previous_next_race'] = [
                ...
                'mrl_race_url' => (string)($nextRace['mrl_race_url'] ?? ''),
                'start_at' => (string)($nextRace['start_at'] ?? ''),
                'final_race_id' => (string)($final['race_id'] ?? ''),
    ...
    if (empty($nextRace)) {
        $futureRace = cms_schedule_first_future_mrl_race($raceSchedule, $now);
        if (!empty($futureRace)) {
            $futureRace['_scheduler_override_reason'] = 'no stored next race found; selected next future MRL race from schedule list';
            return $futureRace;
        }
    }
